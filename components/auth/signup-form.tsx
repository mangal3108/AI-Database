...
import { z } from 'zod'
import { Loader2, Eye, EyeOff, User, Mail, Lock, Shield, CheckCircle2, ArrowRight } from 'lucide-react'
...
export function SignupForm() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    toast.success('Welcome to Internite AI!')
    ...
    >
      ...
      <div className="bg-[#0D111A]/80 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-8 shadow-2xl shadow-black/80 relative overflow-hidden group">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-1.5 font-medium">Start talking to your data in seconds</p>
        </div>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">or sign up with email</span>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.name && <p className="text-xs text-rose-400 mt-1.5 font-medium">{errors.name.message}</p>}
          </div>
          ...
          <div>
            ...
            {errors.email && <p className="text-xs text-rose-400 mt-1.5 font-medium">{errors.email.message}</p>}
          </div>
          ...
          <div>
            ...
            {errors.password && <p className="text-xs text-rose-400 mt-1.5 font-medium">{errors.password.message}</p>}
                  ...
                  {[1, 2, 3, 4].map(step => (
                    <div
                      ...
                      className={`flex-1 rounded-full transition-all duration-300 ${
                        strength >= step
                          ? strength <= 1 ? 'bg-rose-500' : strength <= 2 ? 'bg-amber-500' : strength <= 3 ? 'bg-indigo-500' : 'bg-emerald-500'
                          : 'bg-slate-800'
                      }`}
                    />
                  ))}
        ...
        <p className="text-[11px] text-slate-500 text-center mt-5 leading-relaxed">
          ...
          <span className="text-slate-400 underline underline-offset-2 cursor-pointer">Terms</span> and{' '}
